...
/** Verilog generation entrypoint for [[MoleTop]].
  ...
  * Makefile, gitignored by the per-project rule). Always uses
  * `useBlackBox = true` --- the synth path needs SB_PLL40_PAD and SB_IO
  * primitive instantiations so yosys can recognise them as iCE40 hard cells;
  * the sim-bypass models exist only for `MoleTopSim`.
  *
