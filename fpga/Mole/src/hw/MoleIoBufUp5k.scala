...
/** SB_IO pad wrappers for Mole's bidirectional bus (sda + scl) on iCE40 UP5K.
  ...
  * }}}
  *
  * The `driveLow && driveHigh` case is a semantic violation: the engine
  * should never simultaneously request both meanings ("pull low" and
  * "drive high") for the same line. The symbol decoder
  * (`BusMode.SymbolDecoder`) produces no `(tx_symbol, BUS_MODE)` combination
  * that asserts both. A SpinalHDL `assert(...)` here catches future
  * regressions during simulation. Note: with the wiring above, both inputs
  * true would actually elaborate to `OE=1, D_OUT_0=1` (PP drive high) -- the
  * assert is enforcing the engine-side invariant, not preventing a literal
  * pad shoot-through.
  *
  * `PIN_TYPE = 6'b101000` decomposes as:
  *   - bits[5:2] = 4'b1010 -> PIN_OUTPUT_TRISTATE: the pad's output stage is
  *     tristateable; OUTPUT_ENABLE controls whether D_OUT_0 reaches the pad
  *     or the pad floats.
  *   - bits[1:0] = 2'b00  -> PIN_INPUT (live, unregistered): D_IN_0 is the
  *     live pad value with no flop in the way, so the engine sees the same
  *     value the bus electrics produced this cycle. Mole synchronises this
  *     into the fabric domain at the engine boundary, not at the pad.
  *
  * Bypass strategy (mirrors `MolePllUp5k.useBlackBox`):
  *   - `useBlackBox = true` (default, synthesis path): instantiate one SB_IO
  *     per line. The pad is an `inout(Analog(Bool()))` that the top-level
  *     wires to its own pad port; nextpnr resolves it onto the physical
  *     package pin via the `.pcf` file.
  ...
  *     driver plus an assumed external pull-up. The pad inout is still
  *     declared (same IO shape both ways) but is left dangling under sim --
  *     no testbench drives it. MoleTopSim drives the engine's `MoleBus`
  *     drivers and samples `read` through this same wrapper.
  *
  * @param useBlackBox
...
/** `SB_IO` primitive --- iCE40 UP5K bidirectional pad cell.
  ...
  * model and the Lattice iCE40 LP/HX/UP family handbook section "sysIO Primitive
  * Definitions".
  *
  ...
  *
  * Omitted production pins (same convention as `SB_PLL40_PAD`: omitted to
  * keep yosys from warning about unconnected nets, since we use neither the
  * DDR path nor the registered I/O modes):
  *   D_OUT_1, D_IN_1     (DDR; PIN_TYPE bit 0 = 0 means no DDR output)
  *   OUTPUT_CLK,
  *   INPUT_CLK,
  *   CLOCK_ENABLE         (registered I/O; PIN_TYPE[1:0] = 00 means no
  *                         input register, so no clock is meaningful)
  *   LATCH_INPUT_VALUE    (input-latch enable; PIN_TYPE[1:0] != 01 makes
  *                         this a don't-care)
  */
