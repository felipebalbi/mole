...
bus exposes. Mirrors `I2cIo` in the I2c example project.

**Files:** `src/hw/OpenDrainBus.scala`.

**Suggested IO:**
```scala
case class OpenDrainBus() extends Bundle with IMasterSlave {
...
**Design notes:**
- `ReadableOpenDrain[Bool]`, **not** `TriState`. See `AGENTS.md`.
- The "release all" helper writes `True` to both `write` fields
  (electrical: NMOS off → external pull-up wins).
- One bundle is reused everywhere; do **not** copy the
  open-drain fields inline into other components.

**Sim:** Step 2 lands `OpenDrainBusSim`, the wired-AND helper
(`bus = a.write & b.write & ...`) reused by every sim that has
more than one participant on a bus. Mirror `I2cIoBus`.
...
operand range; assert reserved opcodes decode to a "trap"
instruction the engine refuses to execute.
