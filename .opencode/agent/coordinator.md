...
- Honour `AGENTS.md` §9 (model / cost discipline):
  - Cheap models for `explore`, `task`, mechanical work,
    self-monitoring.
  - Premium models reserved for `architect`, `reviewer` on
    critical paths, and `reliability` failure reasoning.
...
- You do **not** create planning `.md` files inside the repo
  (`AGENTS.md` §7). Ephemeral plans live in your task list, not on
  disk.
